...
class TraccarClient:
    ...
    def _get_device_by_unique_id(self, unique_id: str) -> Optional[Dict]:
        """
        Find a device by its uniqueId.
        
        Args:
            unique_id: The device's unique identifier
    ...
    def update_device_attributes(
        ...
        """
        Update device attributes via Traccar REST API (no position/location created).
        Used when only telemetry data is available without GPS coordinates.
        
        Args:
            device_id: Device unique ID (Smartcar vehicle ID)
            odometer_km: Odometer reading in kilometers
            battery_level: Battery percentage
            Other args: Additional telemetry data
            
        Returns:
            Tuple of (success: bool, message: str)
        """
        if not self.authenticated:
            logger.warning(f"\u26a0\ufe0f  Not authenticated with Traccar, skipping attribute update for {device_id}")
            return False, "Not authenticated"
        
        try:
            # Ensure device exists - create if needed
            device = self._get_device_by_unique_id(device_id)
            
            if not device:
                logger.info(f"\U0001f4f1 Device {device_id} not found, creating...")
                success, msg, _ = self.create_device(device_id, f"Vehicle {device_id}", device_id)
                if not success:
                    logger.error(f"Failed to create device: {msg}")
                    return False, f"Could not create device: {msg}"
                device = self._get_device_by_unique_id(device_id)
                if not device:
                    return False, "Could not find device after creation"
            ...
            if odometer_km is not None:
                attributes['odometer'] = int(odometer_km * 1000)  # Traccar stores in meters
                attributes['totalDistance'] = int(odometer_km * 1000)
            if battery_level is not None:
                attributes['batteryLevel'] = battery_level
            if battery_range is not None:
                attributes['batteryRange'] = int(battery_range)
            if fuel_level is not None:
                attributes['fuelLevel'] = fuel_level
            if low_voltage_battery is not None:
                attributes['lowVoltageBattery'] = int(low_voltage_battery)
            if is_charging is not None:
                attributes['charge'] = is_charging
            if vin is not None:
                attributes['vin'] = vin
            if doors_status is not None:
                attributes['doorsStatus'] = doors_status
            if windows_status is not None:
                attributes['windowsStatus'] = windows_status
            if is_locked is not None:
                attributes['isLocked'] = is_locked
            if custom_attributes:
